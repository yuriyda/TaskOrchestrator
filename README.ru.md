...
| `Ctrl+C` | Скопировать ссылки на задачи для ИИ-агентов |
| `Ctrl+Z` | Отменить |
...
### Отмена действий
- `Ctrl+Z` откатывает до 5 последних изменений — правки, смену статусов, удаления, перемещения в Планировщике дня
- История отмены намеренно короткоживущая: она хранится только в памяти текущего запуска, и каждый шаг истекает через 12 часов — отмена никогда не вернёт неожиданно состояние другого дня
- История также сбрасывается, когда задачи меняются извне — импорт синхронизации или запись [ИИ-агента](#интеграция-с-ии-агентами), — чтобы устаревшая отмена не стёрла изменения, которых она не видела
...
Скилл для агентов позволяет консольному ИИ-агенту (Claude Code или любому
агенту с доступом к командной строке) управлять задачами напрямую в базе
SQLite — создавать, изменять, завершать, править пакетно и откатывать — со
страховкой: транзакционные пакеты, автоматические бэкапы, проверка
инвариантов, корректные для синхронизации записи и журналируемый откат.
Приложение подхватывает внешние изменения за считанные секунды, поэтому
агенты могут работать и при открытом приложении.

В составе скилла есть и MCP-сервер: Claude Desktop и Claude Cowork могут
создавать задачи из документов и заметок встреч через тот же защищённый
движок.

Чтобы указать агенту на конкретные задачи, выделите их и нажмите `Ctrl+C`
(или выберите «Копировать для ИИ-агента» в контекстном меню) — в буфер лягут
короткие ссылки (`to:7K3MZ Название задачи`), которые агент понимает в любой
команде; тот же код показан в диалоге редактирования задачи.

**Инструкция:** [skills/task-orchestrator-db](skills/task-orchestrator-db/README.md) (English + Русский)
